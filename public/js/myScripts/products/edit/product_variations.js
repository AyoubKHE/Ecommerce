...
function rearrangeVariationsNumbers(from) {
    ...
    for (let i = from - 1; i < allVariations.length; i++) {
        ...
        let inputIdInDatabase = allVariations[i].querySelector('input[id*="-id-in-database"]');
        if(inputIdInDatabase !== null) {
            inputIdInDatabase.id = `variation-${i + 1}-id-in-database`;
        }
...
let originaleVariationsCount = document.querySelectorAll('div[id*="variation-"]').length; // convertir en array

let variationsCount = originaleVariationsCount;
...
let originaleAttributes = allChoosedAttributes.map(function (choosedAttribute) {
    return choosedAttribute.value;
})
...
function removeChoosedAttributeCard(attributeNumber) {

    function updateNew_removed_attributesInputs(attributeName) {
        if(originaleAttributes.includes(attributeName)) {

            removedAttributesInput = document.createElement("input");
            removedAttributesInput.type = "hidden";
            removedAttributesInput.name = "removed_attributes[]";
            removedAttributesInput.value = attributeName;

            document.getElementById("product-variations-form").appendChild(removedAttributesInput);
        }
        else {
            document.querySelector(`input[name="new_attributes[]"][value="${attributeName}"]`).remove();
        }
    }
    ...
    function rearrangeInputNumbers(from) {
        ...
        function insertNewDeleteAttributeButton(i) {
            ...
            deleteAttributeButton.addEventListener("click", function () {
                ...
                removeChoosedAttributeCard(i + 1);
                ...
                // updateNew_removed_attributesInputs(attributeName);
            })
    ...
    let attributeName = document.getElementById(`attribute-card-${attributeNumber}`).querySelector(`input[id*="attribute-"]`).value;
...
let allOriginaleDeleteAttributeButton = document.querySelectorAll(`span[id*="delete-attribute-"]`);
allOriginaleDeleteAttributeButton.forEach(function (originaleDeleteAttributeButton) {

    originaleDeleteAttributeButton.addEventListener("click", function () {
        ...
        attributeNumber = originaleDeleteAttributeButton.id.split("-")[2];

        removeChoosedAttributeCard(attributeNumber);

        // attributeName = document.getElementById(`attribute-card-${attributeNumber}`).querySelector(`input[id*="attribute-"]`).value;
...
let choosedAttributesCount = document.querySelectorAll(`div[id*="attribute-card-"]`).length;
...
document.getElementById("attributes-select").addEventListener("change", function () {
    ...
    if (!isAttributeAddedBefore(this.value)) {
        ...
        if(originaleAttributes.includes(attribute)) {
            document.querySelector(`input[name="removed_attributes[]"][value="${attribute}"]`).remove();
        }
        else {
            newAttributesInput = document.createElement("input");
            newAttributesInput.type = "hidden";
            newAttributesInput.name = "new_attributes[]";
            newAttributesInput.value = attribute;

            document.getElementById("product-variations-form").appendChild(newAttributesInput);
        }
    }
        ...
        function createVariationContainer() {
            ...
            function createDeleteVariationButton() {
                ...
                deleteButton.addEventListener("click", function () {
                    ...
                    removeVariationCard(variationNumber);
...
function removeVariationCard(variationNumber) {

    function changeOnUserInterface() {

        document.querySelector(`div[id*="variation-${variationNumber}"]`).remove();
        variationsCount -= 1;

        rearrangeVariationsNumbers(variationNumber);

        alert("La variation est bien été supprimer");
    }

    function changeOnUserInterfaceAndDatabase() {

        let variationIDInDatabase = document.querySelector(`input[id*="variation-${variationNumber}-id-in-database"]`).value;

        axios({
            method: "DELETE",
            url: `http://127.0.0.1:8000/api/products-variations/${variationIDInDatabase}`,
        })
            .then((response) => {
                console.log(response);
            ...
            .catch((error) => {
                ...
                if (error.status === 401 || error.status === 403) {
                    ...
                    alert("La suppression de la variation a échoué, réessayer plus tard.");
    ...
    if (variationNumber <= originaleVariationsCount) {
    ...
    originaleVariationsCount -= 1;

}

let AllDeleteVariationsButtons = document.querySelectorAll('span[id*="delete-variation-"]');

AllDeleteVariationsButtons.forEach(function (deleteVariationButton) {
    deleteVariationButton.addEventListener("click", function () {

        const variationNumber = Number(deleteVariationButton.id.split("-")[2]);
        removeVariationCard(variationNumber);
